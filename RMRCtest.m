function RMRCtest()
    ...
    %         qdot = pinv(J) * xdot;
    %     else
    %         qdot = pinv(J'*J + 0.01*eye(size(J, 2)))*J'*xdot;                                               % Solve velocitities via RMRC
    %     end
    %     error = zeros(3, steps - 1); % Preallocate error array
    % 
    %     error(:,i) = xdot - J * qdot(:); % Ensure qdot is considered as a column vector
    % 
